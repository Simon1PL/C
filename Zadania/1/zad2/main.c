...
int main(int argc, char ** tasks) {
	...
	if (!strcmp(tasks[1], "report")) { make_raport(); return 1; print_results();}
	if (!atoi(tasks[1])) {
...
void make_raport() {
	...
	my_clock=reset_time(my_clock);
	remove_block(0);
	search_directory("~/SysOpy", "an*", "results.txt");
	...
	my_clock=reset_time(my_clock);
	remove_block(0);
	search_directory("~", "ana*", "results.txt");
	...
	my_clock=reset_time(my_clock);
	remove_block(0);
	search_directory("~", "a*", "results.txt");
